...
class Utils {
    ...
    /**
     * Erstellt einen String der Länge size
     * @param size
     * @return rslt
     */
    public static char[] generate(int size)
    {
        ...
        if(size < 11) return "[BGN][END]".toCharArray();
        else
        {
            rslt = new char[size];
            rslt[0] = '[';
    ...
    private static int getRandom(int bLft, int bRgh)
    {
        ...
        while(occurs != true)
        {
            ...
            if(rnd > bLft && rnd < bRgh) occurs = true;
        }

        if(rnd > 57 && rnd < 65) rnd += 7;
        else if(rnd > 90 && rnd < 97) rnd += 6;
